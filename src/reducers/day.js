...
const updateEventTime = (state, action) => {
    ...
    let newState = { ...state};
    if(!newState.events[eventKey][eventIndex]) newState.events[eventKey][eventIndex] = {};
    newState.events[eventKey][eventIndex].eventTime = action.eventTime;
    return { ...state, events: newState.events};
};

const updateEventEndTime = (state, action) => {
    const eventKey = state.selectedDay.format('DD-MM-YYYY');
    const eventIndex = action.eventIndex;
    let newState = { ...state};
    if(!newState.events[eventKey][eventIndex]) newState.events[eventKey][eventIndex] = {};
    newState.events[eventKey][eventIndex].eventEndTime = action.eventEndTime;
    return { ...state, events: newState.events};
};
...
const Day = (state = initialState, action) => {
	switch(action.type) {
        ...
        case 'UPDATE_EVENT_ENDTIME':
            return updateEventEndTime(state, action);
